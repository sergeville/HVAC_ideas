#!/usr/bin/env python3
"""
Interactive Tank #2 Diagnostic Application
...
class Tank2DiagnosticApp:
    ...
    def run_diagnostic(self):
        ...
        # Welcome screen
        self.print_header("TANK #2 AUTO-FILL DIAGNOSTIC APPLICATION")
        ...
        print("  of Tank #2 auto-fill failure. All responses will be saved.")
        print()
        ...
        self.clear_screen()
        self.print_header("STEP 1: IDENTIFY TANK #2 DIRECTION")
        ...
        print("\n  Tank #2 typically corresponds to Tank B (Input IE)")

        tank_direction = self.ask_question(
            "1.1",
            "Which input controls Tank #2?",
        ...
        if "Not Sure" in tank_direction:
            self.show_recommendation(
                "Check system documentation or consult with facility manager to identify Tank #2."
            )
        ...
        print("  2. Manually activate transfer to Tank #2")
        ...
        self.clear_screen()
        self.print_header("STEP 3: CHECK TANK #2 LEVEL SENSOR")

        self.show_info("Checking the float switch or level sensor that triggers Tank #2 fill.")
        ...
        if "Tank A" in self.responses.get("1.1", {}).get('answer', ''):
            ...
            input_to_check = self.ask_question(
                ...
                options=["ID (Tank A)", "IE (Tank B)"],
                input_type="choice"
            )
            relay_to_check = "M4" if "ID" in input_to_check else "M5"
            input_to_check = "ID" if "ID" in input_to_check else "IE"

        self.show_info(f"Testing Input {input_to_check}")
        print("Procedure:")
        print(f"  1. Connect multimeter to terminal {input_to_check} and common ground")
        print("  2. Trigger low-level condition for Tank #2")
        ...
        if test_timer == "YES":
            print("\nProcedure:")
            print("  1. Trigger Tank #2 fill in Auto mode")
            print("  2. Watch solenoid valve (Q7 typically, or Q6 if Tank #2 = Tank A)")
        ...
        system_working = self.ask_question(
            ...
            options=["YES - Tank #2 fills automatically", "NO - Still not working", "Not tested yet"],
            input_type="choice"
        )
    ...
    def save_text_report(self) -> str:
        ...
        filename = f"Tank2_Diagnostic_Session_{self.session_id}.txt"
        filepath = os.path.join(self.output_dir, filename)

        with open(filepath, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("TANK #2 AUTO-FILL DIAGNOSTIC REPORT\n")
            f.write("=" * 80 + "\n\n")
