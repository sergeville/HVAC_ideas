...
CURRENT PROBLEM: Tank #1 does not fill automatically (but may work in manual mode).
...
DIAGNOSTIC APPROACH:
1. First determine if Tank #1 = Tank A (Input ID) or Tank B (Input IE)
2. Test in Manual mode to isolate AUTO vs physical problems
3. Check level sensor/float switch for Tank #1
...
"""
    ...
    def print_header(self):
        ...
        print("\n" + "=" * 80)
        print("  AI-POWERED TANK #1 DIAGNOSTIC ASSISTANT")
        print("=" * 80)
    ...
    def run_diagnostic(self):
        ...
        print("I'll guide you through troubleshooting Tank #1's auto-fill problem.")
        print("\nTips:")
        ...
        initial_prompt = (
            "I'm starting a diagnostic session for Tank #1 auto-fill failure. "
...
Format each section clearly with headers. Be specific and reference actual measurements/observations from our conversation."""
        ...
        report_file = os.path.join(
            self.output_dir,
            f"Tank1_AI_Diagnostic_Report_{self.session_id}.txt"
        )

        with open(report_file, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("AI-POWERED DIAGNOSTIC REPORT - TANK #1 AUTO-FILL FAILURE\n")
            f.write("=" * 80 + "\n\n")
    ...
    def _save_conversation(self):
        ...
        filename = f"Tank1_AI_Conversation_{self.session_id}.txt"
        filepath = os.path.join(self.output_dir, filename)

        with open(filepath, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("AI DIAGNOSTIC CONVERSATION LOG\n")
            f.write("=" * 80 + "\n\n")
            f.write(f"Session ID: {self.session_id}\n")
        ...
        json_file = os.path.join(self.output_dir, f"Tank1_AI_Conversation_{self.session_id}.json")
